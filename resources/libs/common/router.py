...
class Router:
    ...
    def dispatch(self, handle, paramstring):
        ...
        # SETUP CHECKLIST (widget source for homescreen)
        # Use this path as your widget source: plugin://plugin.program.kieranwizard/?mode=checklist
        elif mode == 'checklist':
            acc = accounts.Accounts()
            acc.get_checklist_listing()
            self._finish_widget(handle)

        # ACCOUNTS / POST-INSTALL SETUP
        elif mode == 'accounts':
            acc = accounts.Accounts()
            if action == 'menu':
                acc.post_install_loop()
            elif action == 'fenlight_rd':
                acc.fenlight_rd()
            elif action == 'fenlight_trakt':
                acc.fenlight_trakt()
            ...
            # Homescreen checklist actions (called via RunPlugin from skin shortcuts)
                ...
                acc.checklist_fenlight_trakt()
            elif action == 'checklist_tmdb_trakt':
                acc.checklist_tmdb_trakt()
            elif action == 'checklist_finish':
                acc.checklist_finish()
            elif action == 'checklist_reset':
                acc.checklist_reset()
    ...
    def _finish_widget(self, handle):
        """
        Lightweight finish for widget/non-interactive contexts.
        Skips Container.SetViewMode() which errors in widget contexts.
        """
        xbmcplugin.setContent(handle, 'files')
