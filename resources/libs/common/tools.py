...
def kill_kodi(msg=None, over=None):
    ...
    if choice == 1:
        from resources.libs.common import logging
        plat = str(platform())
        logging.log("Force Closing Kodi: Platform[{0}]".format(plat))
        try:
            if plat == 'windows':
                os.system("taskkill /F /PID %s" % os.getpid())
                # Fallback purely via exec just in case
            ...
            logging.log("Error Force Closing Kodi: {0}".format(e), level=xbmc.LOGERROR)
            os._exit(1)
...
def clean_text(text):
    return text.replace('\n', '')\
                ...
                .replace('icon=""', 'icon="http://"')\
                .replace('fanart=""', 'fanart="http://"')\
                .replace('preview=""', 'preview="http://"')\
                .replace('info=""', 'info="http://"')\
                .replace('minor=""', 'minor="http://"')\
