...
def check_build(name, ret):
    ...
    link = response.text.replace('\n', '').replace('\r', '').replace('\t', '')\
        .replace('gui=""', 'gui="http://"').replace('theme=""', 'theme="http://"').replace('icon=""', 'icon="http://"') \
        .replace('fanart=""', 'fanart="http://"').replace('preview=""', 'preview="http://"').replace('info=""', 'info="http://"') \
        .replace('minor=""', 'minor="http://"').replace('adult=""', 'adult="no"')
...
def check_build_update():
    ...
    link = response.text.replace('\n', '').replace('\r', '').replace('\t', '') \
        .replace('icon=""', 'icon="http://"').replace('fanart=""', 'fanart="http://"')
...
def build_count():
    ...
    if not response:
        return total, count_ver, adultcount, hidden
